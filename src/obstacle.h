...
class OBS{
    ...
    OBS(float xPoz, float yPoz, sf::Texture &winTexture1,sf::Texture &winTexture2,
        ...
        switch(texture){
            ...
            case 1:
            if(!winTexture2.loadFromFile("../assets/win2.png")){throw std::runtime_error("win texture error!");}
            ...
            case 4:
            if(!cpuTexture.loadFromFile("../assets/gameIcon.png")){throw std::runtime_error("win texture error!");}
            this->obs.setTexture(cpuTexture);
            obs.setScale(1.5f, 1.5f);
            break;

            case 5:
            if(!gpuTexture.loadFromFile("../assets/gpu1.png")){throw std::runtime_error("win texture error!");}
            this->obs.setTexture(gpuTexture);
            obs.setScale(1.4f, 1.4f);
            break;

            case 6:
            if(!ssdTexture.loadFromFile("../assets/ssd1.png")){throw std::runtime_error("win texture error!");}
            this->obs.setTexture(ssdTexture);
            obs.setScale(1.6f, 1.6f);
            break;

            case 7:
            if(!mbTexture.loadFromFile("../assets/motherboard1.png")){throw std::runtime_error("win texture error!");}
            this->obs.setTexture(mbTexture);
            obs.setScale(1.5f, 1.7f);
            break;

            case 8:
            if(!virusTexture.loadFromFile("../assets/virus.png")){throw std::runtime_error("win texture error!");}
            this->obs.setTexture(virusTexture);
            obs.setScale(0.2f, 0.2f);
            break;

            case 9:
            if(!virusTexture2.loadFromFile("../assets/virus2.png")){throw std::runtime_error("win texture error!");}
